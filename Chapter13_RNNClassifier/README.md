...
- [RNN 与 LSTM 人名分类实验](./NameClassifier_RNN_LSTM/README.md)

## 实验概览

实验包含两个保持相同数据处理、训练轮数和优化器设置的 Notebook：
...
![RNN 与 LSTM 测试准确率对比](./NameClassifier_RNN_LSTM/images/rnn_lstm_test_accuracy_comparison.png)

> 上表和曲线来自 Notebook 中已经保存的真实运行输出。代码在每一轮都使用 `names_test.csv.gz` 计算准确率并选择最佳模型，因此这里的“测试集”实际上同时承担了验证集的作用；这些数字适合比较本次实验中的两个模型，但不应当视为独立最终测试集上的无偏估计。
...
- 对比 `nn.RNN` 的单一隐藏状态与 `nn.LSTM` 的隐藏状态、细胞状态。
